	...
	useEffect(() => {
		{
			if (searchText) {
				const findSearchDonations = donations.filter((donation) =>
					donation.category.includes(searchText),
				);
			...
			{searchDonation.length === 0 && (
				<div className=" flex gap-5 flex-col text-center justify-center items-center">
					...
					<p className=" font-medium">
						Category names will begin with and uppercase letter like
						<span className=" bg-indigo-600 text-white px-2 py-1 rounded ml-2">
							{" "}
							Health
						</span>
					</p>
